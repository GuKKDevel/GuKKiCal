...
public class GuKKiCalvAlarm extends GuKKiCalvComponent {
	 ...
	 * once.
	 */
	private GuKKiCalcProperty ACTION = null;
	private GuKKiCalcProperty TRIGGER = null;
	/*
	...
	private GuKKiCalcProperty REPEAT = null;
	/*
	 * The following is OPTIONAL, but MUST NOT occur more than once.
	 */
	...
	public GuKKiCalvAlarm() {
		...
		this.kennung=GuKKiCalcKennung.ALARM;
		if (logger.isLoggable(logLevel)) {
	...
	/**
	 * 
	 * @param vCalendar.Name
	 * @param vEvent.UID     vTodo.UID vJournal.UID
	 * @param vEvent.SEQ     vTodo.SEQ vJournal.SEQ
	 * @param vEvent.RECURID vTodo.RECURID vJournal.RECURID
	 * 
	 * @throws GuKKiCalF_FalscheKomponente
	 */
	protected void abschliessen(String pName, String pUID, String pSEQ, String pRECURID)
			throws GuKKiCalF_FalscheKomponente {
		if (logger.isLoggable(logLevel)) {
			logger.log(logLevel, "begonnen");
		}
		String tempACTION = this.ACTION==null? "" :this.ACTION.getWert();
		String tempTRIGGER = this.TRIGGER==null? "":this.TRIGGER.getWert();
		this.schluessel = new GuKKiCalcSchluessel(this.kennung, pName, pUID, pSEQ, pRECURID, tempACTION, tempTRIGGER);
	...
	protected void neueZeile(String zeile) /* throws GuKKiCalException */ {
		...
		if (zeile.length() > 6 && zeile.substring(0, 6).equals("ACTION")) {
			ACTION = new GuKKiCalcProperty(zeile, "ACTION");
			...
			REPEAT = new GuKKiCalcProperty(zeile, "REPEAT");
			...
			TRIGGER = new GuKKiCalcProperty(zeile, "TRIGGER");
	...
	/**
	 * Diese Methode kopiert die iCalendar-Komponente GuKKiCalvAlarm und gibt
	...
	/**
	 * Vergleichen aller Attribute der Komponente GuKKiCalvAlarm Version V 0.0.3
	...
	/**
	 * Gibt sämtliche Daten des vAlarm aus
	 */
	public String toString(String ausgabeLevel) {
		return "Alarm-Identifikation=" +  this.schluessel.toString()+nz;
